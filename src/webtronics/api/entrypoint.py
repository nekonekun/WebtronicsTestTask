...
class ProductionLifeSpanBuilder(LifeSpanBuilder):
    ...
    async def lifespan(self, app: FastAPI):
        ...
        if self.redis_url:
            ...
            reaction_repo = ReactionRepoWithCache(
                sessionmaker=sessionmaker, redis_client=redis_client
            )
        ...
        if self.email_hunter_api_key:
            email_hunter_api = EmailHunterAPI(
                api_key=self.email_hunter_api_key
            )
...
def _main(
    ...
    redis: Annotated[
        ...
        typer.Option(
            envvar='WT_REDIS_URL',
            help='Optional. Example: redis://127.0.0.1:6379/0',
        ),
    ] = None,
    email_hunter_api_key: Annotated[
        str, typer.Option(envvar='WT_HUNTER_KEY', help='Optional.')
    ] = None,
    ...
    app = build_app(
        ...
        lifespanbuilder=ProductionLifeSpanBuilder(
            database, secret_key, redis, email_hunter_api_key
        ),
    )
